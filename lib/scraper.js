...
const processCons = (cons, fc) => {
  ...
  pagecount[cons] = 0
  consolecount += 1
  console.log(`Processing ${cons} [${consolecount} of ${_.keys(counts).length}]`)
  const cc = counts[cons]
  const romids = _.range(cc)
  const pages = _.range(_.chunk(romids, 15).length)
  const pp = new progress.Bar()
  pp.start(pages.length, 0)
    ...
    if (err) def.reject(err)
      ...
      try {
        ...
        async.each(results, (r, nextr) => {
          ...
          pagecount[cons] ++
          logger.debug(`   Processed page ${parseInt(pagecount[cons])} of ${pages.length}`)
          if (err) def.reject(err)
          else def.resolve()
        })
      } catch (err) {
